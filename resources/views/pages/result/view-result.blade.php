...
<div class="space-y-6">
    <!-- Header Section -->
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl shadow-xl p-6 transform transition duration-500 hover:scale-[1.01]">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium 
                                {{ $result->total_score >= 75 ? 'text-green-600' : 
                                   ...
                                   ($result->total_score >= 40 ? 'text-yellow-600' : 'text-red-600')) }}">
                                {{ $result->total_score ?? '-' }}
                            </td>
                        ...
                        <div class="overflow-x-auto rounded-xl shadow-inner">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors {{ $index % 2 === 0 ? 'bg-blue-50' : '' }}">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">
                                        ...
                                        @foreach ($subjects as $subject)
                                            @php
                                                ...
                                                $textClass = $score >= 75 ? 'text-green-600' : 
                                                            ($score >= 60 ? 'text-blue-600' : 
                                                            ($score >= 40 ? 'text-yellow-600' : 'text-red-600'));
                                            @endphp
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium {{ $textClass }}">
                                                {{ $score ?? '-' }}
                                            </td>
                                        @endforeach
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                            @php
                                                $avgClass = $student->average_score >= 75 ? 'bg-green-100 text-green-800' : 
    ...
    function setMode(mode) {
        ...
        buttons.forEach(btn => {
            if (btn.getAttribute('onclick').includes(mode)) {
                btn.classList.add('bg-gradient-to-r', 'from-indigo-600', 'to-purple-600', 'text-white', 'shadow-lg', 'transform', 'hover:scale-105');
                btn.classList.remove('bg-gray-100', 'text-gray-700', 'hover:bg-gray-200');
            } else {
                btn.classList.remove('bg-gradient-to-r', 'from-indigo-600', 'to-purple-600', 'text-white', 'shadow-lg', 'transform', 'hover:scale-105');
